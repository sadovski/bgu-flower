...
%% Check expiry date before returning and remove key if outdated.
%% Returns 	{{ok,Data},new_LRU} if found
%%			{none, Original_LRU} otherwise		  			
%% @end
...
%% a few lookups...
%% or in other words, for n > 100000 and x < 20 is
...
calc_expire({Absolute, Sliding}, Now)when Absolute /= undefined, Sliding /= undefined ->
if Absolute < Now + Sliding ->
Absolute;
true -> Now + Sliding
end;

calc_expire({_Absolute, Sliding}, Now) when Sliding /= undefined ->
	Now + Sliding;

calc_expire({Absolute, _Sliding}, _) when Absolute /= undefined ->
	Absolute;
